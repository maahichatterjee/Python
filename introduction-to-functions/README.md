...
**🔗 Problem Link:** [View on NeetCode](https://neetcode.io/problems/python-introduction-to-functions/question)
...
ChallengeIn the code editor, we are calling two functions, greet() and say_goodbye(). The greet() function is already defined but it has a bug. It should print "Hello, World!".

The say_goodbye() function is not defined. Define it so that it prints "Goodbye, World!". How much should we indent code within a function? In Python, indentation is used to define code blocks. All code that belongs to a function should be indented. The standard convention is to use 4 spaces for indentation, which is what we are following in this course. Sometimes code is only indented by 2 spaces, but this is less common. Either way, it's important to be consistent with your indentation. This is the only way Python knows what code belongs to a function.
